...
public abstract class ChessGame {
    ...
    private LinkedList<Memento> memory = new LinkedList();
    ...
    public void flipBoard() {
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 8; j++) {
                Piece temp = board.getPiecesArray()[i][j];
                board.getPiecesArray()[i][j] = board.getPiecesArray()[8 - i - 1][j];
                board.getPiecesArray()[8 - i - 1][j] = temp;
            }
        }
    }

    public int moveChecker(Move move, ChessBoard board) {
    ...
    public void addMemento() {
        memory.push(new Memento(this.board, this.gameStatus, this.whoseTurn, this.lastMove, this.canWhiteCastleKingSide, this.canWhiteCastleQueenSide, this.canBlackCastleKingSide, this.canBlackCastleQueenSide));
    }
    ...
    public void undo() {
        ...
        Memento memento = memory.pop();

        this.board = new ChessBoard(memento.getBoard());
        this.gameStatus = memento.getGameStatus();
        this.whoseTurn = memento.getWhoseTurn();
        this.lastMove = memento.getLastMove();
        this.canWhiteCastleKingSide = memento.isCanWhiteCastleKingSide();
        this.canWhiteCastleQueenSide = memento.isCanWhiteCastleQueenSide();
        this.canBlackCastleKingSide = memento.isCanBlackCastleKingSide();
        this.canBlackCastleQueenSide = memento.isCanBlackCastleQueenSide();
    }

    public void setFirstUndo() {
        firstUndo = true;
    }
}
